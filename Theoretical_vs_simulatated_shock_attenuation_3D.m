...
source.p_mask(x2_pos, x2_pos, x2_pos) = 1;    % position of 2nd source [grid points]
...
for i = 0:0.25:num_of_sensor
    sensor_pos(j) = find(abs(kgrid.x_vec - (xsh*i)) < 0.0025);
    j = j + 1;
    disp(i)
end
...
sensor.mask(500, 500, 500) = 1;
sensor.record = {'p_final'};


%% SIMULATION

input_arg = {'PMLInside', false, 'DataCast', 'single'};
sensor_data = kspaceFirstOrder3D(kgrid, medium, source, sensor, input_arg{:});
...
figure(1)
plot(kgrid.x_vec, sensor_data.p_max, 'b', 'LineWidth', 2)
...
hold on
plot(kgrid.x_vec, pa, 'r', 'LineWidth', 2)  % envelope over wave profile
hold off
legend('P max', 'Analytical');
...
P_exi = 15e+6;
xsh = (0.3068*5e+6)/P_exi;
pa_15  = (P_exi * pi) ./ (1 + abs(x/xsh));
